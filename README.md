...
- PDF 확대/축소
  ...
  - `Ctrl + 마우스 휠`
  - 화면 맞춤
- 시험 시작 시 PDF는 한 페이지 전체가 아니라 페이지의 약 1/4 영역이 보이도록 크게 표시됩니다.
- 시험 목록의 `시험 시작`, `정답 편집`, `기록`, `삭제`는 아이콘 버튼으로 표시되며, 마우스를 올리면 설명이 표시됩니다.
- 모바일 PWA 시험 화면은 문제와 답안을 한 번에 볼 수 있도록 압축 2열 레이아웃을 유지하고, 타이머/채점 컨트롤을 답안 패널 상단에 고정합니다.
- Electron 설치 파일 빌드
- PWA 빌드 및 브라우저 설치 지원
...
PWA는 Vercel 정적 사이트로 배포할 수 있습니다.

저장소에는 [vercel.json](C:/Code/ETC/CBT/vercel.json)이 포함되어 있습니다.
...
1. GitHub/GitLab/Bitbucket에 이 프로젝트를 push합니다.
2. Vercel에서 해당 저장소를 Import합니다.
3. Vercel이 `vercel.json`을 읽어 PWA 빌드를 실행합니다.
4. 이후 main 브랜치에 push할 때마다 자동으로 새 버전이 배포됩니다.

로컬 PC를 켜둘 필요는 없습니다. Vercel이 클라우드에서 빌드하고 호스팅합니다.
